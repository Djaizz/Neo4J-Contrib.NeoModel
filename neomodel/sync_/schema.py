"""
Schema management for the neomodel module.

``SchemaManager`` installs and drops indexes and constraints for node and
...
``QueryRunner`` and reads server version facts from an
``ConnectionManager``.
"""
...
from neomodel.sync_.connection import ConnectionManager
...
class SchemaManager:
    ...
    def __init__(
        self, connection: ConnectionManager, query_runner: QueryRunner
    ) -> None:
        self.db: ConnectionManager = connection
        self._query: QueryRunner = query_runner
    ...
    def list_indexes(self, exclude_token_lookup: bool = False) -> list[dict]:
        ...
        """
        indexes, meta_indexes = self._query.cypher_query("SHOW INDEXES")
    ...
    def list_constraints(self) -> list[dict]:
        ...
        """
        constraints, meta_constraints = self._query.cypher_query(
            LIST_CONSTRAINTS_COMMAND
        )
    ...
    def drop_constraints(
        ...
        results, meta = self._query.cypher_query(LIST_CONSTRAINTS_COMMAND)
    ...
    def _create_node_fulltext_index(
        ...
        if self.db.version_is_higher_than(VERSION_FULLTEXT_INDEXES_SUPPORT):
            label = target_cls.__label__
            ...
            """
            try:
                self._query.cypher_query(query)
            except ClientError as e:
                if e.code in (
                    RULE_ALREADY_EXISTS,
                    INDEX_ALREADY_EXISTS,
            ...
            raise FeatureNotSupported(
                f"Creation of full-text indexes from neomodel is not supported for Neo4j in version {self.db.database_version}. Please upgrade to Neo4j 5.16 or higher."
            )
    ...
    def _create_node_vector_index(
        ...
        if self.db.version_is_higher_than(VERSION_VECTOR_INDEXES_SUPPORT):
            label = target_cls.__label__
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes from neomodel is not supported for Neo4j in version {self.db.database_version}. Please upgrade to Neo4j 5.15 or higher."
            )
    ...
    def _create_relationship_vector_index(
        ...
        if self.db.version_is_higher_than(VERSION_RELATIONSHIP_VECTOR_INDEXES_SUPPORT):
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes for relationships from neomodel is not supported for Neo4j in version {self.db.database_version}. Please upgrade to Neo4j 5.18 or higher."
            )
    ...
    def _create_relationship_constraint(
        ...
        if self.db.version_is_higher_than(VERSION_RELATIONSHIP_CONSTRAINTS_SUPPORT):
            ...
            raise FeatureNotSupported(
                f"Unique indexes on relationships are not supported in Neo4j version {self.db.database_version}. Please upgrade to Neo4j 5.7 or higher."
            )
